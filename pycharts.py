...
#    save_to_txt()
#    xie_zheng('./xie_zheng.txt', './out_file.txt')
    #读取城市数据
    city = []
    with open('out_file.txt',mode='r',encoding='utf-8') as f:
        rows = f.readlines()
        for row in rows:
            if len(row.split(',')) == 5:
    ...
    #data = [(u'南昌', 158), (u'肥城', 15), (u'玉溪', 23)]
    print(data)
    
    geo = Geo("《邪不压正》粉丝人群地理位置","数据来源：恋习Python", title_color = "#fff", title_pos = "center", width = 1200,
